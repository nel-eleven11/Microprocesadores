...
int main(){
	...
	//Se inicializa el atributo de los hilos
	//y se setea el atributo de los hilos como joinable
	for(int i = 0; i < 10; i++){
	...
	for(int i = 0; i < 10; i++){
		...
		pthread_create(&thread_id[i], &attr[i], calculos, (void*) (intptr_t) i);
	}
	...
	for(int i = 0; i < 10; i++){
		...
		pthread_join(thread_id[i], &exit_value);
